...
//for messages
#include <motoman_sia20d_moveit_updated/trajPlot.h>
#include <motoman_sia20d_moveit_updated/trajPlotPoint.h>
#include <motoman_sia20d_moveit_updated/trajPlotPointInfo.h>
...
const std::string IK_PLANNING_GROUP = "ik_plan_group";
...
static bool getJacobianFromIK(robot_state::RobotState *test_state, const Eigen::Affine3d& test_end_effector_state, Eigen::MatrixXd& jacobian, int indexToSet, double Value,const robot_state::JointModelGroup *joint_model_group, const robot_state::JointModelGroup *ik_joint_model_group)
{
      //GET JOINT VALUES
    ...
    // ROS_INFO_STREAM("Translation: \n" << test_end_effector_state.translation() << "\n");
    // ROS_INFO_STREAM("Rotation: \n" << test_end_effector_state.rotation() << "\n");

    std::size_t attempts = 10; //10
    double timeout = 0.1; //0.1
    test_state->setVariablePosition(indexToSet, Value);
    bool found_ik = test_state->setFromIK(ik_joint_model_group, test_end_effector_state, attempts, timeout);
    ...
    test_state->getJacobian(joint_model_group,
                                 test_state->getLinkModel(joint_model_group->getLinkModelNames().back()),
...
int main(int argc, char** argv) {
  ...
  //     http://docs.ros.org/indigo/api/moveit_ros_planning/html/classrobot__model__loader_1_1RobotModelLoader.html
    ...
    const robot_state::JointModelGroup* ik_joint_model_group = robot_state->getJointModelGroup(IK_PLANNING_GROUP);
    ...
    ros::Publisher plot_publisher = node_handle.advertise<motoman_sia20d_moveit_updated::trajPlot>("trajectory_plot",1);
    ...
    {
      std::cout<<"Waiting for Marker to connect to /trajectory_viewer_main/visualization_marker"<<std::endl;
      ros::Duration(0.5).sleep();
    }
...
// {
  ...
  for(int main_loop_iter = 0; main_loop_iter <max_Iter; main_loop_iter++) { //change number of iterations
   ...
   //^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    {
      start_name ="start_state_easy";
      end_name = "goal_state_easy";
    }
    else
    {
    ...
    /*
      ...
      Following lists code for collision-checking IK

      https://github.com/ros-planning/moveit_visual_tools/blob/kinetic-devel/src/imarker_robot_state.cpp



      Following lists code for setFromIK
...
// static bool getJacobianFromIK(robot_state::RobotState *test_state, Eigen::Affine3d& test_end_effector_state, Eigen::MatrixXd *jacobian, int indexToSet, double Value,const robot_state::JointModelGroup *joint_model_group, const robot_state::JointModelGroup *ik_joint_model_group)
    const Eigen::Affine3d& test_end_effector_state = test_state.getGlobalLinkTransform(Camera_Link);
    bool succJac = getJacobianFromIK(&test_state, test_end_effector_state, jacobian, 0, 0.20, joint_model_group,ik_joint_model_group);
    ...
    // double timeout = 0.1;
    // test_state.setVariablePosition(0,0.25);
    // bool found_ik = test_state.setFromIK(ik_joint_model_group, test_end_effector_state, attempts, timeout);
    ...
    // Eigen::Affine3d pose_test_transform = test_state.getFrameTransform(Camera_Link);
    // // get FrameTransform() to get frame transform from tool0 to base link
    // // using frametransform for setFromIK, emphasizing the frame
    // pose_test_msg = test_state.setFromIK(ik_joint_model_group, pose_test_transform,10,)
    //USE setFromIK
    ...
    */
    ...
    /////////////////////////
      ...
      //////////////////
      //PUBLISH MESSAGE to Trajectory Viewer
      //^^^^^^^^^^^^^^^
      /////////////////

      std::vector<motoman_sia20d_moveit_updated::trajPlotPoint> trajPlotPoints;

      for(auto iter = 0; iter < response.trajectory.joint_trajectory.points.size(); iter++)
      {
        
        //TODO: ADD Pose to the message!
        motoman_sia20d_moveit_updated::trajPlotPoint thisPlotPoint;
        thisPlotPoint.point_number = iter;

        int disc = 20;

        robot_state::RobotState pointState(robot_model);

        pointState.setVariablePositions(response.trajectory.joint_trajectory.points[iter].positions);
        pointState.update();

        const Eigen::Affine3d& test_end_effector_state = pointState.getGlobalLinkTransform(Camera_Link);

          //THE FOLLOWING ADDS THE TRAJECTORY POINT as the first data point for easy pose
          //^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
          motoman_sia20d_moveit_updated::trajPlotPointInfo statePlotPointInfoMsg;
          statePlotPointInfoMsg.joint_value = pointState.getVariablePosition(0);
          Eigen::MatrixXd statePlotPointJacobian;
          Eigen::Vector3d ref_pp(0.0, 0.0, 0.0);
          pointState.getJacobian(joint_model_group,
                                 pointState.getLinkModel(joint_model_group->getLinkModelNames().back()),
                                 ref_pp, statePlotPointJacobian);
          statePlotPointInfoMsg.JacobianDeterminant = statePlotPointJacobian.determinant();
          statePlotPointInfoMsg.collision_free = true;
          statePlotPointInfoMsg.isPoint = true;
          thisPlotPoint.pointInfo.push_back(statePlotPointInfoMsg);


          //FOLLOWING CODE PRINTS the state values
          //^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
          // std::vector<double> test_joint_values;

          // pointState.copyJointGroupPositions(joint_model_group, test_joint_values);
          // for (std::size_t i = 0; i < joint_names.size(); ++i)
          // {
          //   ROS_INFO("Joint %s: %f", joint_names[i].c_str(), test_joint_values[i]);
          // }

          // std::cout<<statePlotPointJacobian.determinant()<<std::endl;


        //THE FOLLOWING DETERMINES THE CONTOUR PLOT
        //^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        for(int j = 0; j<= disc; j++)
        {

          motoman_sia20d_moveit_updated::trajPlotPointInfo trajPlotPointInfoMsg;

          //TODO: FIGURE OUT HOW TO OBTAIN JOINT LIMITS
          double lower_bound = -3.1415;
          double upper_bound = 3.1415;
          double joint_value = lower_bound + (upper_bound-lower_bound)/disc * j;
          bool isPoint = false;
      ...
      // static bool getJacobianFromIK(robot_state::RobotState *test_state, Eigen::Affine3d& test_end_effector_state, Eigen::MatrixXd *jacobian, int indexToSet, double Value,const robot_state::JointModelGroup *joint_model_group, const robot_state::JointModelGroup *ik_joint_model_group)
          ...
          bool succJac = getJacobianFromIK(&pointState, test_end_effector_state, jacobian, 0, joint_value, joint_model_group,ik_joint_model_group);

          // UNCOMMENT FOLLOWING TO SEE THE setFromIK state
          //^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
          // moveit_msgs::PlanningScene temp_plan_scene_msg;
          // moveit_msgs::RobotState temp_robot_state_msg;

          // moveit::core::robotStateToRobotStateMsg(pointState, temp_robot_state_msg);
          // temp_plan_scene_msg.robot_state = temp_robot_state_msg;
          // temp_plan_scene_msg.is_diff = true;
          // planning_scene_diff_publisher.publish(temp_plan_scene_msg);
          ...
          bool collision_free;
          bool isPlotPoint = false;
          double j_det;
          if(succJac)
          {
            collision_free = true; 
            //TODO: Actually check if this state is collision free!
            j_det = jacobian.determinant();
            // std::cout<<"j_det :: "<<j_det<<std::endl;
          } else
          {
            collision_free = false;
            j_det = std::numeric_limits<double>::quiet_NaN();
          }

          // trajPlotPointInfoMsg.joint_value.push_back(joint_value);
          // trajPlotPointInfoMsg.JacobianDeterminant.push_back(j_det);
          // trajPlotPointInfoMsg.collision_free.push_back(collision_free);
          // trajPlotPointInfoMsg.isPoint.push_back(isPlotPoint);

          trajPlotPointInfoMsg.joint_value = joint_value;
          trajPlotPointInfoMsg.JacobianDeterminant = j_det;
          trajPlotPointInfoMsg.collision_free = collision_free;
          trajPlotPointInfoMsg.isPoint = isPlotPoint;


          thisPlotPoint.pointInfo.push_back(trajPlotPointInfoMsg);

        }
        std::cout<<"Finished plotting trajectory iteration :: "<<iter<<"\n";
        trajPlotPoints.push_back(thisPlotPoint);
      }

      motoman_sia20d_moveit_updated::trajPlot trajPlotMsg;
      trajPlotMsg.joint = 0;
      trajPlotMsg.points = trajPlotPoints;

      plot_publisher.publish(trajPlotMsg);


      visual_tools.prompt("Message sent; Press next to continue");
